...
<head>
    ...
    <style>
        * {
        ...
        .nota-credito-row {
            color: #c0392b; /* Rojo sutil para identificar devoluciones */
        }
...
<body>
    ...
    <div class="info-box">
        <table class="info-table">
            <tr>
                ...
                <td class="value-right">{{ \Carbon\Carbon::parse($filtros['desde'])->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($filtros['hasta'])->format('d/m/Y') }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="label-right">MONEDA</td>
                <td class="value-right">Dólares (USD)</td>
            </tr>
        </table>
    </div>
    ...
    <div class="resumen-box">
        <div class="saldo-principal">
            Saldo Total al Corte: ${{ number_format($saldo_total, 2) }}
        </div>
    </div>
    ...
    <table class="movimientos-table">
        <thead>
            <tr>
                ...
                <th class="text-right" style="width: 15%;">Saldo Acum.</th>
            </tr>
        </thead>
        <tbody>
            <tr class="saldo-anterior-row">
                <td>{{ \Carbon\Carbon::parse($filtros['desde'])->format('d/m/Y') }}</td>
                <td><strong>SALDO ANTERIOR (Balance histórico)</strong></td>
                <td class="text-right">-</td>
                <td class="text-right">-</td>
                <td class="text-right"><strong>${{ number_format($saldo_anterior, 2) }}</strong></td>
            </tr>
            @foreach($movimientos as $mov)
            <tr class="{{ $mov['descripcion'] === 'Nota de Crédito' ? 'nota-credito-row' : '' }}">
            ...
            <tr style="background: #808080; color: white; font-weight: bold;">
                <td colspan="4" class="text-right">SALDO FINAL PENDIENTE</td>
    ...
    <div style="margin-top: 20px; font-size: 8px;">
        <p><strong>Nota:</strong> Los valores en la columna Crédito (-) incluyen pagos realizados y notas de crédito aplicadas.</p>
    </div>
    
    <div class="fecha-generacion">
        ERP System - Generado por TJC Logistics el {{ now()->format('d/m/Y H:i:s') }}
    </div>
